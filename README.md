...
- Windows x64 便携包：`LinguaGacha-FE-v0.103.0-fe.4-win-x64.zip`
...
启用 FE 项目后，侧边栏会出现“PSP 画面预览”。预览器使用中性的 480×272 全屏
Canvas，不假设所有文本都进入同一种对话框，并支持：

- 上一条、下一条、逐条浏览；
- 搜索、文件筛选和警告筛选；
- 原文与译文上下对照，译文可直接编辑并实时刷新画面；
- 保存、重新翻译、清空译文和设置翻译状态；
- 状态及校对警告标签；
- 从者和性别条件组合切换；
...
画布显示当前分支的测量宽度与物理行数，但两者仅为中性参考，不产生通用画面溢出
状态。详情栏显示存储分类、游戏编码字节数、槽位容量、剩余或超出字节数以及超长和
重定位权限。控制符、换行、槽位容量、共享存储和未解析存储仍可作为 FE 规则警告显示，
但普通警告只需确认，不会阻止导出。
校对列表保留正文的真实换行，长文本会自动换行并按内容增高，不再压缩为单行省略显示。

## 容量感知翻译

FE 适配按逻辑文本从 `extra_field.__linguagacha_fe_v1.classification` 构造逐条约束，同时
提供给普通 API 与 SakuraLLM。约束只包含分类、槽位容量、超长/重定位权限及必要地址
限制，不发送文件路径或索引。

长度按最终译文的实际游戏编码字节数计算：优先使用 `chinese-glyph-codec.json`，再参考
日版原生字形映射；尚未入库的新字符按未来扩展编码的 2 字节计算。物理换行和原样写入
的控制符文本参与计数，NUL 终止符与槽位填充不参与计数。

- 原文与译文的物理行数、空行位置及换行边界必须严格一致，通用重试阈值不能放行。
- `slot_capacity=null` 不进行容量重试。
- `allow_overlength=true` 允许超过原槽位，但仍要求表达简洁。
- 禁止超长的条目只重试违规项；提示携带上一版译文、实际字节数、容量及至少需缩短
  的字节数。
- 最多自动重试 3 次；仍超限时保留最后一版草稿，将状态设为 `ERROR` 并显示“需要人工
  缩短”，不会静默截断。
- 手动保存超限草稿仍允许；导出生成非阻断 `FE_STORAGE_CAPACITY` 警告。
...
游戏编码与 UTF-8 长度差异、多行逻辑文本共享单一容量、物理换行硬校验、容量定向重试
与第三次失败保留草稿、所有条件分支渲染、六份译文主/Ruby 字库零缺字、编码槽耗尽
安全失败，以及两种索引导出结构。
