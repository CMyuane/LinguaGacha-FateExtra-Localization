...
- 预览 Canvas 固定为中性的 480×272 全屏画布；共享布局器提供 Ruby、颜色、变量、图标、字号、偏移和条件分支计算，并把宽度与物理行数作为测量值返回，不作通用画面异常判定。
- `/api/toolbox/fate-extra/items` 返回存储分类、实际游戏编码字节数、槽位容量、剩余/超出字节数及超长/重定位权限；renderer 展示这些后端派生事实，不自行复制编码表。缺字和不可编码字符不属于校对状态；未同步字符以同尺寸 Noto Sans CJK 临时字形显示。
