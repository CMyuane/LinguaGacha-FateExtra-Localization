...
除通用矩阵外，FE 变更必须验证索引解析与双模式导出、游戏编码字节计算、物理换行硬
校验、逐条容量提示、只重试违规条目、第三次失败保留草稿并标记 `ERROR`、允许超长与
无容量分支、所有从者/性别分支渲染、字库映射/纹理/manifest 一致性、编码槽耗尽安全
失败，以及六份当前译文主字库和 Ruby 字库零缺字。发布前还要确认仓库未跟踪 `.lg`、
分类 SQLite、完整译文、ISO、EBOOT、日志、缓存和 `build` 目录。
