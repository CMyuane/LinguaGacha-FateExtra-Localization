...
除通用矩阵外，FE 变更必须验证索引解析与双模式导出、18/19 全宽字边界、第四行溢出、
所有从者/性别分支、字库映射/纹理/manifest 一致性、编码槽耗尽安全失败，以及六份当前
译文主字库和 Ruby 字库零缺字。发布前还要确认仓库未跟踪 `.lg`、分类 SQLite、完整
译文、ISO、EBOOT、日志、缓存和 `build` 目录。
